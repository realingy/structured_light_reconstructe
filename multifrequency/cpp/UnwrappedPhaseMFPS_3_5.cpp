...
void CalImageWrappedPhase(const int step); // 包裹相位计算
	...
	for(int n = 0; n < 3; n++)
	{
		...
		for (int i = 0; i < gHeight; i++)
		{
			for (int j = 0; j < gWidth; j++)
			{
				...
				{
					imageWrappedPhase[n].at<float>(i,j) = atan( (2*I2 - 2*I4) / (2*I3 - I1 - I5) ) + CV_PI;
				}
				else if ( 2*I3 > I1+I5 && I2 > I4) //第一象限
				{
					imageWrappedPhase[n].at<float>(i,j) = atan( (2*I2 - 2*I4) / (2*I3 - I1 - I5) );
				}
				else if ( 2*I3 > I1+I5 && I2 < I4) //第四象限
				{
					imageWrappedPhase[n].at<float>(i,j) = atan( (2*I2 - 2*I4) / (2*I3 - I1 - I5) ) + 2*CV_PI;
				}
			}
		}
	}
...
// 包裹相位计算
void CalImageWrappedPhase(const int step)
{
	switch (step)
	{
	case 5:
		CalImageWrappedPhase_5();
	default:
		break;
	}
...
int main()
{
	...
	CalImageWrappedPhase(step); // 包裹相位计算
